...
class ModelConfig(BaseConfig):
    ...
    z_max: float = Field(default=120.0, gt=0.0)
    ...
    def _validate_optional_heads(self) -> "ModelConfig":
        enabled = [
            name
            for name, active in (
                ("segmentation_head", self.segmentation_head),
                ("use_grouppose_keypoints", self.use_grouppose_keypoints),
                ("depth_head", self.depth_head),
            )
            if active
        ]
        if len(enabled) > 1:
            raise ValueError(f"Optional output heads are mutually exclusive; enabled: {', '.join(enabled)}")
        return self
...
class TrainConfig(BaseConfig):
    ...
    depth_loss_coef: float = Field(default=5.0, ge=0.0)
    pinhole_loss_coef: float = Field(default=1.0, ge=0.0)
    focal_length_px: float | None = Field(default=None, gt=0.0)
    ball_diameter_m: float = Field(default=0.22, gt=0.0)
    ball_class_ids: list[int] = Field(default_factory=list)
    curriculum_phase1_epochs: int = Field(default=10, ge=0)
    curriculum_phase2_epochs: int = Field(default=40, ge=0)
...
class DepthTrainConfig(TrainConfig):
    """Training configuration for RF-DETR models with metric-depth output.

    The depth and optional pinhole-consistency loss knobs live on :class:`TrainConfig` so ``RFDETR.train()`` can
    validate them for any model variant.  This subclass is a convenience that enables the depth head by default when
    constructing config-native training pipelines.
    """
