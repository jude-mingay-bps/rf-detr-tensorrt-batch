...
class RFDETRModelModule(LightningModule):
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if self.model_config.depth_head:
            pinhole_weight = (
                0.0
                if self.current_epoch < self.train_config.curriculum_phase1_epochs
                else self.train_config.pinhole_loss_coef
            )
            for name in self.criterion.weight_dict:
                if name == "loss_pinhole" or name.startswith("loss_pinhole_"):
                    self.criterion.weight_dict[name] = pinhole_weight
        ...
        if self.train_config.compute_train_metrics:
            with torch.no_grad():
                ...
                inference_outputs = {
                    ...
                    if k in ("pred_logits", "pred_boxes", "pred_masks", "pred_keypoints", "pred_depth")
