...
class PostProcess(nn.Module):
    ...
    def forward(
        ...
        out_depth = outputs.get("pred_depth")
        self._validate_outputs(out_logits, out_masks, out_keypoints, out_depth, target_sizes)
        ...
        if out_depth is not None:
            return self._postprocess_depth(out_depth, scores, labels, boxes, topk_boxes)
    ...
    def _postprocess_depth(
        ...
        """Attach metric-depth predictions aligned with selected queries."""
        return [
            {
                "scores": score,
                "labels": label,
                "boxes": box,
                "depth": depth.index_select(0, query_indices),
            }
            for depth, score, label, box, query_indices in zip(
                out_depth, scores, labels, boxes, topk_boxes, strict=True
            )
        ]
