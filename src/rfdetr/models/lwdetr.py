...
class LWDETR(nn.Module):
    ...
    def __init__(
        ...
        if self.depth_head_enabled:
            ...
            depth_out_layer = cast(nn.Linear, self.depth_embed.layers[-1])
            nn.init.constant_(depth_out_layer.weight.data, 0)
            nn.init.constant_(depth_out_layer.bias.data, 0)
    ...
    def forward(self, samples: NestedTensor, targets: list[dict[str, Tensor]] | None = None) -> dict[str, Any]:
        ...
        if hs is not None:
            ...
            outputs_depth = self.depth_embed(hs).sigmoid() * self.z_max if self.depth_head_enabled else None
        ...
        if self.two_stage:
            ...
            depth_enc = self.depth_embed(hs_enc).sigmoid() * self.z_max if self.depth_head_enabled else None
            keypoints_enc = None
            ...
            if hs is not None:
                ...
                if depth_enc is not None:
                    out["enc_outputs"]["pred_depth"] = depth_enc
            else:
    ...
    def forward_export(self, tensors: Tensor) -> tuple[Tensor, ...]:
        ...
        if hs is not None:
            ...
            if self.depth_head_enabled:
                outputs_depth = self.depth_embed(hs_enc).sigmoid() * self.z_max
            if self.use_grouppose_keypoints and enc_kp_predictions is not None:
                outputs_keypoints = self._format_keypoint_output(
                    enc_kp_predictions,
                    enc_kp_predictions.shape[0],
        ...
        if outputs_depth is not None:
            return outputs_coord, outputs_class, outputs_depth
        else:
    ...
    def _set_aux_loss(
        ...
        outputs_depth: Tensor | None = None,
    ) -> list[dict[str, Any]]:
        ...
        if outputs_depth is not None:
            names.append("pred_depth")
            values.append(outputs_depth[:-1])
...
def build_criterion_and_postprocessors(args: "BuilderArgs") -> tuple[SetCriterion, PostProcess]:
    ...
    has_depth = getattr(args, "depth_head", False)
    if has_depth:
    ...
    focal_length_px: float | None = getattr(args, "focal_length_px", None)
    ball_diameter_m: float = getattr(args, "ball_diameter_m", 0.22)
    ball_class_ids: list[int] = getattr(args, "ball_class_ids", [])
    resolution: int = getattr(args, "resolution", 640)
    if args.segmentation_head:
