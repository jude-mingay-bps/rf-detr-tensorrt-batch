...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        ball_class_ids: list[int] | None = None,
        resolution: int = 640,
    ) -> None:
    ...
    def loss_depth_distill(
        ...
        **kwargs: Any,
        ...
        """Regress teacher-provided metric depth for matched detections."""
        del kwargs
        assert "pred_depth" in outputs
        idx = self._get_src_permutation_idx(indices)
        src_depth = outputs["pred_depth"][idx]
        target_depth = torch.cat(
            [target["depth"][target_indices] for target, (_, target_indices) in zip(targets, indices)],
            dim=0,
        ).to(src_depth)
    ...
    def loss_pinhole(
        ...
        **kwargs: Any,
        ...
        """Constrain ball depth using its apparent width and a pinhole camera model."""
        del num_boxes, kwargs
        assert "pred_depth" in outputs
        zero = outputs["pred_depth"].sum() * 0.0
        if self.focal_length_px is None or not self.ball_class_ids:
            return {"loss_pinhole": zero}
        ...
        for class_id in self.ball_class_ids:
            ball_mask |= target_labels == class_id
        if not torch.any(ball_mask):
            return {"loss_pinhole": zero}

        width_pixels = src_boxes[ball_mask, 2:3] * self.resolution
        pinhole_depth = self.focal_length_px * self.ball_diameter_m / width_pixels.clamp_min(1e-6)
        loss = F.smooth_l1_loss(src_depth[ball_mask], pinhole_depth.detach(), reduction="none")
        return {"loss_pinhole": loss.mean()}
