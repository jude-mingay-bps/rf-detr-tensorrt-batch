...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    Args:
        ...
        include_depth: If ``True``, read optional per-annotation metric depths
            into the target dict under the ``"depth"`` key.
...
class ConvertCoco:
    ...
    - ``"depth"`` – ``(N, 1)`` float32 tensor of per-instance metric depths,
      only present when ``include_depth=True``.
    ...
    Args:
        ...
        include_depth: If ``True``, read the optional per-annotation ``depth``
            value into an ``(N, 1)`` float tensor. Missing values become zero.
    ...
    def __call__(self, image: Image.Image, target: dict[str, Any]) -> tuple[Image.Image, dict[str, Any]]:
        ...
        if self.include_depth:
            depths = torch.as_tensor([obj.get("depth", 0.0) for obj in anno], dtype=torch.float32).reshape(-1, 1)
            target["depth"] = depths[keep]
