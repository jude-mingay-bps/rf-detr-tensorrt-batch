    ...
    def export(
        ...
        *,
        ...
        try:
            ...
            elif getattr(self.model_config, "depth_head", False):
        ...
        **kwargs: Any,
        ...
        if isinstance(predictions, tuple):
            ...
            if len(predictions) == 3:
                ...
                elif getattr(getattr(self.model, "args", None), "use_grouppose_keypoints", False):
        ...
        for i, result in enumerate(results):
            ...
            if "masks" in result:
                ...
                depth = result["depth"][keep]
                detections = Detections(
                    xyxy=boxes.float().cpu().numpy(),
                    confidence=scores.float().cpu().numpy(),
                    class_id=labels.cpu().numpy(),
                    data={"depth_m": depth.squeeze(-1).float().cpu().numpy()},
                )
            else:
                detections = Detections(
                    xyxy=boxes.float().cpu().numpy(),
