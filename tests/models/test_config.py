...
class TestModelConfigValidation:
    ...
    def test_depth_head_is_opt_in(self, sample_model_config) -> None:
        config = ModelConfig(**sample_model_config)
        assert config.depth_head is False
        assert config.z_max == 120.0

    def test_optional_heads_are_mutually_exclusive(self, sample_model_config) -> None:
        with pytest.raises(ValidationError, match="mutually exclusive"):
            ModelConfig(**sample_model_config, depth_head=True, segmentation_head=True)

    def test_depth_train_config_defaults(self) -> None:
        config = DepthTrainConfig(dataset_dir="/tmp")
        assert config.depth_head is True
        assert config.depth_loss_coef == 5.0
        assert config.pinhole_loss_coef == 1.0
